...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="navegacao">
		<xsl:element name="table">
			<xsl:attribute name="id">tabelaImoveisNavegacaoEmp</xsl:attribute>
					...
					<xsl:when test="count(navegacao/paginas/pagina) &gt; 0">
						<xsl:if test="number(navegacao/paginas/ini) &gt; 1">
							<xsl:element name="td">
								...
								<xsl:element name="a">
									...
									<xsl:attribute name="onclick">PROMOCOESE.carregaPagina(0);</xsl:attribute>
									&lt;&lt;
								</xsl:element>
							</xsl:element>
						</xsl:if>
							...
							<xsl:element name="td">
								...
								<xsl:choose>
									<xsl:when test="number(valor)!=number(../../pagina)">
										<xsl:element name="a">
											<xsl:attribute name="href">javascript:void(0);</xsl:attribute>
											<xsl:attribute name="onclick">PROMOCOESE.carregaPagina(<xsl:value-of select="valor"/>);</xsl:attribute>
						...
						<xsl:if test="number(navegacao/paginas/fim/valor) &gt; 1">
							<xsl:element name="td">
								...
								<xsl:element name="a">
									<xsl:attribute name="href">javascript:void(0);</xsl:attribute>
									<xsl:attribute name="onclick">PROMOCOESE.carregaPagina(<xsl:value-of select="number(navegacao/paginas/fim/valor)-1"/>);</xsl:attribute>
									&gt;&gt;
								</xsl:element>
							</xsl:element>
						</xsl:if>
	...
	<xsl:template name="imo">
		...
		<xsl:element name="div">
			<xsl:attribute name="id">cardImovelEmp<xsl:value-of select="position()"/></xsl:attribute>
			<xsl:attribute name="class">cardImovel</xsl:attribute>
			<xsl:element name="div">
				<xsl:attribute name="id">divTitEmp<xsl:value-of select="position()"/></xsl:attribute>
				<xsl:attribute name="class">divTit</xsl:attribute>
				<xsl:element name="a">
					<xsl:attribute name="href">&#35;<xsl:value-of select="referencia"/></xsl:attribute>
					<xsl:attribute name="onclick">exibirDetalhes(&#39;cardImovelEmp<xsl:value-of select="position()"/>&#39;,<xsl:value-of select="referencia"/>,<xsl:copy-of select="$TIPO_VISUALIZA"/>,PROMOCOESE.getAnchor());</xsl:attribute>
			...
			<xsl:element name="div">
				<xsl:attribute name="id">divFotoEmp<xsl:value-of select="position()"/></xsl:attribute>
			...
			<xsl:element name="div">
				<xsl:attribute name="id">panelAnuEmp<xsl:value-of select="position()"/></xsl:attribute>
				<xsl:attribute name="class">panelAnu</xsl:attribute>
				<xsl:element name="div">
					<xsl:attribute name="id">panelHeaderAnuEmp<xsl:value-of select="position()"/></xsl:attribute>
					<xsl:attribute name="class">panelHeader</xsl:attribute>
					<xsl:value-of select="regiao"/>
				</xsl:element>
				<xsl:element name="div">
					<xsl:attribute name="id">panelContentAnuEmp<xsl:value-of select="position()"/></xsl:attribute>
			...
			<xsl:if test="number($EXIBIRINFORMACOES)>0">
				<xsl:element name="div">
					<xsl:attribute name="id">panelValEmp<xsl:value-of select="position()"/></xsl:attribute>
					<xsl:attribute name="class">panelVal</xsl:attribute>
					<xsl:element name="div">
						<xsl:attribute name="id">panelHeaderValEmp<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelHeader</xsl:attribute>
						Valores
					</xsl:element>
					<xsl:element name="div">
						<xsl:attribute name="id">panelContentValEmp<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelContent</xsl:attribute>
						<xsl:element name="table">
							<xsl:attribute name="id">tabelaValorEmp<xsl:value-of select="position()"/></xsl:attribute>
				...
				<xsl:element name="div">
					<xsl:attribute name="id">panelDetEmp<xsl:value-of select="position()"/></xsl:attribute>
					<xsl:attribute name="class">panelDet</xsl:attribute>
					<xsl:element name="div">
						<xsl:attribute name="id">panelHeaderDetEmp<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelHeader</xsl:attribute>
						Detalhes
					</xsl:element>
					<xsl:element name="div">
						<xsl:attribute name="id">panelContentDetEmp<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelContent</xsl:attribute>
						<xsl:element name="table">
							<xsl:attribute name="id">tabelaDetEmp<xsl:value-of select="position()"/></xsl:attribute>
							<xsl:attribute name="class">tabelaValor</xsl:attribute>
							<xsl:element name="tbody">
