...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="navegacao">
		<xsl:element name="table">
			<xsl:attribute name="id">tabelaImoveisNavegacaoLoc</xsl:attribute>
					...
					<xsl:when test="count(navegacao/paginas/pagina) &gt; 0">
						<xsl:if test="number(navegacao/paginas/ini) &gt; 1">
							<xsl:element name="td">
								...
								<xsl:element name="a">
									...
									<xsl:attribute name="onclick">PROMOCOESL.carregaPagina(0);</xsl:attribute>
									&lt;&lt;
								</xsl:element>
							</xsl:element>
						</xsl:if>
							...
							<xsl:element name="td">
								...
								<xsl:choose>
									<xsl:when test="number(valor)!=number(../../pagina)">
										<xsl:element name="a">
											<xsl:attribute name="href">javascript:void(0);</xsl:attribute>
											<xsl:attribute name="onclick">PROMOCOESL.carregaPagina(<xsl:value-of select="valor"/>);</xsl:attribute>
						...
						<xsl:if test="number(navegacao/paginas/fim/valor) &gt; 1">
							<xsl:element name="td">
								...
								<xsl:element name="a">
									<xsl:attribute name="href">javascript:void(0);</xsl:attribute>
									<xsl:attribute name="onclick">PROMOCOESL.carregaPagina(<xsl:value-of select="number(navegacao/paginas/fim/valor)-1"/>);</xsl:attribute>
									&gt;&gt;
								</xsl:element>
							</xsl:element>
						</xsl:if>
	...
	<xsl:template name="imo">
		...
		<xsl:element name="div">
			<xsl:attribute name="id">cardImovelLoc<xsl:value-of select="position()"/></xsl:attribute>
			<xsl:attribute name="class">cardImovel</xsl:attribute>
			<xsl:element name="div">
				<xsl:attribute name="id">divTitLoc<xsl:value-of select="position()"/></xsl:attribute>
				<xsl:attribute name="class">divTit</xsl:attribute>
				<xsl:element name="a">
					<xsl:attribute name="href">&#35;<xsl:value-of select="referencia"/></xsl:attribute>
					<xsl:attribute name="onclick">exibirDetalhes(&#39;cardImovelLoc<xsl:value-of select="position()"/>&#39;,<xsl:value-of select="referencia"/>,<xsl:copy-of select="$TIPO_VISUALIZA"/>,PROMOCOESL.getAnchor());</xsl:attribute>
			...
			<xsl:element name="div">
				<xsl:attribute name="id">divFotoLoc<xsl:value-of select="position()"/></xsl:attribute>
			...
			<xsl:element name="div">
				<xsl:attribute name="id">panelAnuLoc<xsl:value-of select="position()"/></xsl:attribute>
				<xsl:attribute name="class">panelAnu</xsl:attribute>
				<xsl:element name="div">
					<xsl:attribute name="id">panelHeaderAnuLoc<xsl:value-of select="position()"/></xsl:attribute>
					<xsl:attribute name="class">panelHeader</xsl:attribute>
					<xsl:value-of select="regiao"/>
				</xsl:element>
				<xsl:element name="div">
					<xsl:attribute name="id">panelContentAnuLoc<xsl:value-of select="position()"/></xsl:attribute>
			...
			<xsl:if test="number($EXIBIRINFORMACOES)>0">
				<xsl:element name="div">
					<xsl:attribute name="id">panelValLoc<xsl:value-of select="position()"/></xsl:attribute>
					<xsl:attribute name="class">panelVal</xsl:attribute>
					<xsl:element name="div">
						<xsl:attribute name="id">panelHeaderValLoc<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelHeader</xsl:attribute>
						Valores
					</xsl:element>
					<xsl:element name="div">
						<xsl:attribute name="id">panelContentValLoc<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelContent</xsl:attribute>
						<xsl:element name="table">
							<xsl:attribute name="id">tabelaValorLoc<xsl:value-of select="position()"/></xsl:attribute>
				...
				<xsl:element name="div">
					<xsl:attribute name="id">panelDetLoc<xsl:value-of select="position()"/></xsl:attribute>
					<xsl:attribute name="class">panelDet</xsl:attribute>
					<xsl:element name="div">
						<xsl:attribute name="id">panelHeaderDetLoc<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelHeader</xsl:attribute>
						Detalhes
					</xsl:element>
					<xsl:element name="div">
						<xsl:attribute name="id">panelContentDetLoc<xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="class">panelContent</xsl:attribute>
						<xsl:element name="table">
							<xsl:attribute name="id">tabelaDetLoc<xsl:value-of select="position()"/></xsl:attribute>
							<xsl:attribute name="class">tabelaValor</xsl:attribute>
							<xsl:element name="tbody">
